...
const ReviewFiles = () => {
  ...
  const deleteFie = async (id) => {
    try {
      const response = await orderService.deleteFile({ id: id })
      fetchAllFiles()
      openSuccessNotification('Xoá file thành công!')
    } catch (error) {
      openErrorNotification(error.data?.message ?? error.message)
    }
  }

  const acceptFile = async (id) => {
    try {
      const response = await orderService.acceptFile({ id: id })
      fetchAllFiles()
      openSuccessNotification('Review file thành công!')
    } catch (error) {
      openErrorNotification(error.data?.message ?? error.message)
    }
  }
  const fetchAllFiles = async () => {
    try {
      const response = await orderService.showAllFilesNotAccepted(searchQuery)
      ...
      <CRow>
        <CCol xs>
          <CCard className="mb-4">
            <CCardHeader className="fw-bold fs-5">Danh sách file cần kiểm duyệt </CCardHeader>
            <CCardBody>
              <CTable align="middle" className="mb-0 border" hover responsive>
                <CTableHead className="text-nowrap bg-primary">
                  <CTableRow>
                    ...
                    <CTableHeaderCell className="bg-body-tertiary">Người đóng góp</CTableHeaderCell>
                  ...
                  {registrationOrders.length > 0 ? (
                    registrationOrders.map((order, index) => (
                      <CTableRow key={order.id}>
                        ...
                        <CTableDataCell>{order.updated_at}</CTableDataCell>
                        <CTableDataCell>{order.is_approved ? 'Kiểm duyệt' : 'Đang kiểm duyệt'}</CTableDataCell>
