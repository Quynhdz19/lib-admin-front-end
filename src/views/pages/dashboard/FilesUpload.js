...
const FilesUploaded = () => {
  ...
  const uploadFiles = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        openSuccessNotification('upload file thành công!')
        setFile(null)
        setSubject('')
        setClassLevel('')
        fetchAllFiles()
  ...
  const fetchAllFiles = async () => {
    try {
      const response = await orderService.getAllFile(searchQuery)
      ...
      <CRow>
        <CCol xs>
          <CCard className="mb-4">
            <CCardHeader className="fw-bold fs-5">Tải files</CCardHeader>
            ...
            <CCardHeader className="fw-bold fs-5">Danh sách file </CCardHeader>
            <CCardBody>
                  ...
                  {registrationOrders.length > 0 ? (
                    registrationOrders.map((order, index) => (
                      <CTableRow key={order.id}>
                        ...
                        <CTableDataCell>
                          {order.is_approved ? 'Đã kiểm duyệt' : 'Đang kiểm duyệt'}
                        </CTableDataCell>
